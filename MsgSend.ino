...
/*********************************************************
 * sendBeacon()
 *    Send the XY position to TwoPotatoe
 *********************************************************/
void sendBeacon() {
  send2Msg(SEND_XPOS, 2, currentMapLoc.x);
  send2Msg(SEND_YPOS, 2, currentMapLoc.y);
  xTransmitRequest(XBEE_DEST_2P, rf2Data, rf2DataPtr);
  rf2DataPtr = 0;
//  Serial.println(timeMilliseconds / 1000.0);
}
...
void xAdd2Message(int cmd, char buf[], int len) {
  if ((len + 1 + rf2DataPtr) >= RF_DATA_SIZE) return;
  rf2Data[rf2DataPtr++] = cmd;
  for (int i = 0; i < len; i++) {
    rf2Data[rf2DataPtr++] = buf[i];
  }
}
